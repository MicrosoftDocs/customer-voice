...
9.	In the **Select action** list, choose one of the following actions:
    ...
    - **Show**: Select a question to be displayed per the response to a question.
    - **Hide**: Select a question to be hidden per the response to a question.
    - **Toggle**: Toggle the state of the selected question per the response to a question.
...
10.	In the **Select target** list, choose a target for the selected action. If you select **Show**, **Hide,** or **Toggle** as the action, you can select **Question** as the target. If you select **Navigate to** as the action, you can choose one of the following targets:
    ...
    - **Question**: Skip to a question per the response to a question. The questions in between the source and target questions are hidden from the responder.
    - **End of survey**: End the survey per the response to a question.
    - **Chain survey**: Open a different survey, created by you, per the response to a question.
    - **URL**: Open a website per the response to a question. You must add `http://` to the URL for it to work properly.
...
12.	Select **Add “If false”** to add the action that is triggered when the defined condition doesn't meet the criteria. Then follow steps 8 through 11.
